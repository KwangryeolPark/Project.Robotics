...
class DQNAgent:
    def __init__(self):
        self.model = nn.Sequential(
            nn.Linear(4, 1024),#신경망은 카트 위치, 카트 속도, 막대기 각도, 막대기 속도까지 4가지 정보를 입력
            nn.ReLU(),
    ...
    def act(self, state):#행동 선택(담당)함수
        ...
        if random.random() > eps_threshold:# 무작위 값 > 앱실론값 : 학습된 신경망이 옳다고 생각하는 쪽으로,
            return self.model(state.cuda()).data.max(1)[1].view(1, 1)
    ...
    def learn(self):#메모리에 쌓아둔 경험들을 재학습(replay)하며, 학습하는 함수
        ...
        current_q = self.model(states.cuda()).gather(1, actions)
        
        max_next_q = self.model(next_states.cuda()).detach().max(1)[0]#에이전트가 보는 행동의 미래 가치(max_next_q)
